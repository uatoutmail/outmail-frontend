...
beforeEach(() => {
  ...
  api.get.mockImplementation((url) => {
    ...
    if (url === '/api/outreach/companies') return Promise.resolve({ data: { companies: [company], pagination: { total: 1, totalPages: 1 } } });
    if (url === '/api/agent/status') return Promise.resolve({ data: { online: true } });
    if (url === '/api/outreach/status') return Promise.resolve({ data: { emails: [], counts: {} } });
...
describe('ColdOutreachTab — run-outreach gating', () => {
  ...
  it('queues outreach and shows a success toast naming the company on success', async () => {
    ...
    await waitFor(() => expect(api.post).toHaveBeenCalledWith('/api/outreach/single', { companyEmailId: 'contact1' }));
    ...
    api.get.mockImplementation((url) => {
      ...
      if (url === '/api/outreach/companies')
        return Promise.resolve({
          data: { companies: [{ ...company, contacts: [{ ...company.contacts[0], contacted: true }] }], pagination: { total: 1, totalPages: 1 } },
        });
      return Promise.resolve({ data: {} });
    });
    render(<ColdOutreachTab />);
  ...
  it('loads and renders My Outreach history on tab switch', async () => {
    api.get.mockImplementation((url) => {
      ...
      if (url === '/api/outreach/status')
        return Promise.resolve({
          data: { emails: [{ id: 'e1', recipient_email: 'hr@acme.com', status: 'sent', lane: 'single' }], counts: { sent: 1 } },
        });
      return Promise.resolve({ data: {} });
    });
    render(<ColdOutreachTab />);
