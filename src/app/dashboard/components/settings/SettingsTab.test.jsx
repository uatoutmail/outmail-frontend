...
function mockGetImpl({ gmailConnected = false, emailUsage = null, resumes = [] } = {}) {
  api.get.mockImplementation((url) => {
    ...
    if (url === '/api/user/gmail/status') return Promise.resolve({ data: { connected: gmailConnected } });
...
describe('SettingsTab — profile save', () => {
  it('saves profile changes and shows a success toast', async () => {
    ...
    await waitFor(() => expect(updateUserMock).toHaveBeenCalledWith(expect.objectContaining({ display_name: 'Jane Renamed', name: 'Jane Renamed' })));
  ...
  it('shows the backend error when the save reports failure', async () => {
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith('Failed to update profile: Name already in use'));
  });
  ...
  it('rejects a second save within the 2-second cooldown window', async () => {
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith('Please wait a moment before saving again'));
...
describe('SettingsTab — resume management', () => {
  it('rejects an oversized file without calling the API', async () => {
    const { toast } = await import('sonner');
    render(<SettingsTab />);
    const bigFile = new File(['x'.repeat(11 * 1024 * 1024)], 'resume.pdf', { type: 'application/pdf' });
    const input = document.querySelector('input[type="file"]');
    await userEvent.upload(input, bigFile);

    expect(toast.error).toHaveBeenCalledWith(expect.stringContaining('File size must be less than'));
  ...
  it('uploads a valid resume and adds it to the list', async () => {
    ...
    await waitFor(() => expect(api.post).toHaveBeenCalledWith('/api/resumes/', expect.any(FormData), expect.any(Object)));
  ...
  it('shows the backend error message when an upload fails', async () => {
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith('Upload failed: Virus scan failed'));
  });
  ...
  it('opens the attachment URL when viewing, and errors when there is none', async () => {
    mockGetImpl({ resumes: [{ id: 'r1', filename: 'resume.pdf', s3_path: 'https://s3.example.com/resume.pdf' }] });
    const openSpy = vi.fn();
...
describe('SettingsTab — auto-mailing toggle', () => {
  it('blocks enabling auto-mailing when Gmail is not connected', async () => {
    ...
    await userEvent.click(screen.getByText('Auto-Mailing').closest('.mt-8').querySelector('button'));

    expect(toast.error).toHaveBeenCalledWith('Please connect your Gmail account before enabling Auto-Mailing');
    expect(updateUserMock).not.toHaveBeenCalled();
  });
  ...
  it('rolls back the optimistic toggle when persisting auto-mailing fails', async () => {
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith('Could not update auto-mailing: server unavailable'));
...
describe('SettingsTab — institution', () => {
  ...
  it('shows a placeholder when no institution is joined', async () => {
    render(<SettingsTab />);
    await waitFor(() => expect(screen.getByText(/haven.t joined an institution yet/i)).toBeInTheDocument());
  });
});
