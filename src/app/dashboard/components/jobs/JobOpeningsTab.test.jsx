...
const strongJob = { id: 'j1', title: 'Backend Engineer', company: 'Acme', matchScore: 82, reasons: [], signals: [] };
const goodJob = { id: 'j2', title: 'Frontend Engineer', company: 'Beta', matchScore: 60, reasons: [], signals: [] };
const possibleJob = { id: 'j3', title: 'QA Engineer', company: 'Gamma', matchScore: 30, reasons: [], signals: [] };

function jobsResponse(jobs, overrides = {}) {
  return { data: { success: true, data: jobs, pagination: { page: 1, limit: 10, total: jobs.length, totalPages: 1, ...overrides } } };
}
...
describe('JobOpeningsTab — job actions', () => {
  it('applies to a job: opens the link, records the action, and marks it applied', async () => {
    ...
    await waitFor(() => expect(api.post).toHaveBeenCalledWith('/api/jobs/j1/interactions', { action: 'applied' }));
  ...
  it('shows an error toast and leaves the job unchanged when recording the action fails', async () => {
    ...
    const { toast } = await import('sonner');
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith('Could not discard this job. Please try again.'));
  ...
  it('auto-applies: opens the link and records the action', async () => {
    api.get.mockResolvedValue(jobsResponse([{ ...strongJob, applyLink: 'https://apply.example.com/j1' }]));
    api.post.mockResolvedValue({});
    const openSpy = vi.fn();
    vi.stubGlobal('open', openSpy);
    render(<JobOpeningsTab />);
  ...
  it('resets an applied job\'s status back to pending', async () => {
    ...
    render(<JobOpeningsTab />);
    await waitFor(() => expect(screen.getByRole('button', { name: /reset status/i })).toBeInTheDocument());

    await userEvent.click(screen.getByRole('button', { name: /reset status/i }));

    await waitFor(() => expect(api.patch).toHaveBeenCalledWith('/api/jobs/j1/status', { status: 'pending' }));
  });
});
...
describe('JobOpeningsTab — filter and pagination', () => {
  ...
  it('paginates: Next fetches the next page', async () => {
    ...
    await waitFor(() => expect(api.get).toHaveBeenCalledWith('/api/jobs', { params: { page: 2, limit: 10 } }));
