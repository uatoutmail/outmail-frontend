      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            <span
              className={`text-xs font-medium px-2 py-0.5 rounded-full bg-white/5 ${getStatusColor(job.userAction)}`}
            >
        ...
        <div className="flex items-center bg-white/5 rounded-xl px-4 py-3 border border-white/10 shrink-0 self-start md:self-center">
          <div className="flex flex-col items-center">
            <span className={`text-2xl font-bold ${getPriorityScoreColor(job.matchScore)}`}>
              {job.matchScore}
            </span>
      ...
      {job.reasons && job.reasons.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {job.reasons.map((r, i) => (
            <span
              key={i}
              className="flex items-center gap-1 px-2.5 py-1 bg-green-500/10 border border-green-500/20 text-green-200 rounded-md text-[11px] font-medium"
            >
      ...
      {skills.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {skills.map((signal, i) => (
            <span
              key={i}
              className="px-2 py-1 bg-purple-500/10 border border-purple-500/20 text-purple-200 rounded-md text-[11px] font-medium"
            >
              {signal}
            </span>
          ))}
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              {job.qualifications && (
                <div className="bg-purple-500/5 rounded-xl p-4 border border-purple-500/10">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-purple-300/60 mb-3">
                    Qualifications
                  </h4>
                  <div
                    className="text-white/70 text-sm prose prose-invert max-w-none prose-p:leading-relaxed prose-li:my-1"
