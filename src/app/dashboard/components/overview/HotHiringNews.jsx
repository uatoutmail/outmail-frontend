  ...
  useEffect(() => {
    const fetchInsights = async () => {
      try {
        ...
        // The empty state below already reads correctly for "no signals to
        // show", so a failure falls through to it rather than inventing a
        // second failure UI. Recorded for us, invisible to the user.
        logger.error("Failed to fetch hiring insights", err);
        setNews([]);
      } finally {
        setLoading(false);
      }
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
          ...
          <p className="text-[12px] text-white/40 font-medium">
            No fresh hiring signals detected for today.
          </p>
          <p className="text-[10px] text-white/25 mt-1 whitespace-pre-wrap">
            Check back tomorrow for AI-curated market alerts.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span
                    className={`text-[9px] px-1.5 py-0.5 rounded-full font-semibold ${item.tagColor}`}
                  >
                    {item.tag}
                  </span>
              ...
              {item.url !== "#" && (
                <ExternalLink
                  size={11}
                  className="text-white/20 group-hover:text-purple-400 transition-colors flex-shrink-0 mt-1"
                />
              )}
            </div>
          ))}
