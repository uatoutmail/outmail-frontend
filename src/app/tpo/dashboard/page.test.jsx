...
vi.mock("next/navigation", () => ({
  ...
  usePathname: () => "/tpo/dashboard",
}));
vi.mock("@/lib/api", () => ({ api: { get: vi.fn() } }));
vi.mock("@/context/AuthContext", () => ({ useAuth: vi.fn() }));
vi.mock("sonner", () => ({ toast: Object.assign(vi.fn(), { success: vi.fn(), error: vi.fn() }) }));
// These have their own data-fetching and are out of scope for a page-level test.
vi.mock("@/component/tpo/TPOOverviewCards", () => ({
  default: ({ stats }) => <div>Overview Cards ({stats ? "loaded" : "pending"})</div>,
}));
vi.mock("@/component/tpo/TPOCharts", () => ({ default: () => <div>Charts Stub</div> }));
vi.mock("@/component/tpo/TPOStudentTable", () => ({
  default: () => <div>Student Table Stub</div>,
}));
vi.mock("@/component/tpo/TPOMentorshipPanel", () => ({
  default: () => <div>Mentorship Panel Stub</div>,
}));
