...
describe("TpoClaimPage", () => {
  ...
  it("shows an error when the link has no invite token at all", async () => {
    ...
    render(<TpoClaimPage />);
    await waitFor(() =>
      expect(screen.getByText(/couldn't claim this invite/i)).toBeInTheDocument()
    );
  ...
  it("claims the invite, refreshes the session, and redirects to the dashboard on success", async () => {
    ...
    await waitFor(() =>
      expect(api.post).toHaveBeenCalledWith("/api/auth/tpo/claim", {
        inviteToken: "signed-token-abc",
      })
    );
  ...
  it("shows the backend error (e.g. wrong signed-in email) and does not redirect on failure", async () => {
    api.post.mockRejectedValue({
      response: {
        data: {
          error:
            "This invite was sent to real-tpo@institute.edu. Sign in with that exact Google account to claim it.",
        },
      },
    });
    ...
    await waitFor(() =>
      expect(screen.getByText(/sign in with that exact google account/i)).toBeInTheDocument()
    );
