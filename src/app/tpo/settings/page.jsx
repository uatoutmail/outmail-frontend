...
const Toggle = ({ checked, onChange }) => (
  ...
  >
    <span
      className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${checked ? "translate-x-5" : "translate-x-0.5"}`}
    />
  </button>
);
...
const Input = ({ value, onChange, placeholder }) => (
  <input
    ...
    className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-purple-400 bg-white placeholder-gray-300"
  />
);
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        logger.error("Failed to fetch admin settings", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <TPOPageShell
        title="Settings"
        subtitle="Manage your institution profile, team, and notification preferences"
      >
      ...
      <div className="space-y-6">
        ...
        <Section icon={Building2} title="Institution Profile">
          ...
          <Field
            ...
            hint="Upload a PNG or SVG — max 500 KB. Shown in reports and student emails."
          >
        ...
        <Section icon={Bell} title="Notification Preferences">
          {[
            {
              key: "weeklyReport",
              label: "Weekly Placement Report",
              desc: "Summary email every Monday with placement metrics",
            },
            {
              key: "studentActivity",
              label: "Student Activity Alerts",
              desc: "Notify when students are inactive for 7+ days",
            },
            {
              key: "jobAlerts",
              label: "New Job Intelligence Alerts",
              desc: "Get notified when new high-priority jobs are added",
            },
            {
              key: "mentorUpdates",
              label: "Mentor Session Updates",
              desc: "Reminders and summaries for upcoming mentor sessions",
            },
            {
              key: "platformUpdates",
              label: "Platform Feature Updates",
              desc: "Learn about new Outmail features and improvements",
            },
          ].map(({ key, label, desc }) => (
            <div
              key={key}
        ...
        <Section icon={Users} title="Team Members">
          <p className="text-xs text-gray-500">
            Admins and Co-TPOs can access all dashboard sections. Only Admins can change settings.
          </p>
            ...
            {members.map((m) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="h-9 w-9 rounded-full bg-purple-200 text-purple-800 text-xs font-bold flex items-center justify-center flex-shrink-0">
                    {m.avatar}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${m.role === "Admin" ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-600"}`}
                  >
                    {m.role}
                  </span>
                  {m.role !== "Admin" && (
                    <button
                      onClick={() => setMembers((ms) => ms.filter((x) => x.email !== m.email))}
          ...
          <div className="flex items-center gap-3 pt-2">
            <Input
              placeholder="colleague@college.ac.in"
              value={newEmail}
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Up to 500 students · Unlimited jobs · All analytics · Priority support
              </p>
              <p className="text-xs text-gray-400 mt-2">
                Renews on <span className="text-gray-700 font-medium">1 Aug 2026</span>
              </p>
            </div>
            <button className="text-xs font-semibold text-purple-600 border border-purple-200 px-4 py-2 rounded-lg hover:bg-purple-50 transition">
              Manage Plan
          ...
          <div className="space-y-3">
            ...
            <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  Connect your institution&apos;s Google Workspace or Azure AD
                </p>
              </div>
              <button className="text-xs font-semibold text-gray-500 border border-gray-200 px-4 py-2 rounded-lg hover:bg-gray-50 transition">
                Configure
              </button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-red-200 shadow-sm overflow-hidden">
          ...
          >
            ...
            <ChevronDown
              size={14}
              className={`transition-transform ${danger ? "rotate-180" : ""}`}
            />
          </button>
          {danger && (
              ...
              <div className="flex items-center justify-between bg-red-50 rounded-xl px-5 py-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Pause all activity; data is retained for 90 days
                  </p>
                </div>
                <button className="text-xs font-semibold text-red-600 border border-red-200 px-4 py-2 rounded-lg hover:bg-red-100 transition">
                  Deactivate
                </button>
              </div>
              <div className="flex items-center justify-between bg-red-50 rounded-xl px-5 py-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Permanently erase all student and placement data
                  </p>
                </div>
                <button className="text-xs font-semibold text-red-600 border border-red-200 px-4 py-2 rounded-lg hover:bg-red-100 transition">
                  Delete Data
