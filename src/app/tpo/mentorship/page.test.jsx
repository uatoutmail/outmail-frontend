...
vi.mock('next/navigation', () => ({ useRouter: () => ({ replace: vi.fn(), push: vi.fn() }), usePathname: () => '/tpo/mentorship' }));
...
describe('MentorshipPage — loading and empty states', () => {
  ...
  it('shows an empty state when there are no sessions', async () => {
    ...
    render(<MentorshipPage />);
    await waitFor(() => expect(screen.getByText(/no mentorship sessions scheduled yet/i)).toBeInTheDocument());
  });
...
describe('MentorshipPage — with sessions', () => {
  ...
  it('renders the rating distribution bars', async () => {
    api.get.mockResolvedValue({ data: [session1, session2] });
    render(<MentorshipPage />);
    await waitFor(() => expect(screen.getByText(/rating distribution \(2 reviews\)/i)).toBeInTheDocument());
  });
});
