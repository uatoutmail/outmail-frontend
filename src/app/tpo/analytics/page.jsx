...
export default function AnalyticsPage() {
  ...
  if (loading) {
    return (
      <TPOPageShell
        title="Analytics"
        subtitle="Deep-dive metrics on student engagement, outreach performance, and placement readiness"
      >
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm mb-6">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-sm font-semibold text-gray-900">
            Student Engagement Trend — Aug 2025 to Mar 2026
          </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900 mb-1">
            Weekly Email Volume (Jan – Mar 2026)
          </h3>
          <p className="text-xs text-gray-400 mb-4">
            Total cold emails sent by your cohort per week
          </p>
          <div className="h-56">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-2">
            Which branches are most engaged with off-campus outreach
          </p>
          <div className="h-44">
            ...
            {branchFunnel?.map((b) => (
              <div key={b.id} className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-gray-600">
                  {b.id} <span className="font-semibold">{b.value}</span>
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="px-6 py-5 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Top 5 students ranked by composite Outmail Engagement Score
          </p>
        </div>
        <div className="divide-y divide-gray-50">
          {leaderboard?.map((s, i) => (
            <div key={s.name} className="px-6 py-4 flex items-center gap-5">
              <span
                className={`text-lg font-black w-6 text-center ${i === 0 ? "text-yellow-400" : i === 1 ? "text-gray-400" : i === 2 ? "text-orange-400" : "text-gray-200"}`}
              >
                {i + 1}
              </span>
              ...
              <div className="text-right">
                ...
                <p
                  className={`text-xs font-medium ${s.delta.startsWith("+") ? "text-green-500" : s.delta.startsWith("-") ? "text-red-400" : "text-gray-400"}`}
                >
                  {s.delta} this week
                </p>
              </div>
