...
vi.mock('next/navigation', () => ({ useRouter: () => ({ replace: vi.fn(), push: vi.fn() }), usePathname: () => '/tpo/analytics' }));
...
const payload = {
  engagementTrend: [
    { id: 'Applications', color: '#000', data: [{ x: 'Week 1', y: 10 }, { x: 'Week 2', y: 20 }] },
    { id: 'Interviews', color: '#111', data: [{ x: 'Week 1', y: 2 }, { x: 'Week 2', y: 3 }] },
  ],
  weeklyEmailData: [{ week: 'Jan 1', sent: 50 }],
  branchFunnel: [{ id: 'CSE', value: 40, color: '#8B5CF6' }, { id: 'ECE', value: 20, color: '#3B82F6' }],
  leaderboard: [
...
describe('AnalyticsPage', () => {
  ...
  it('shows "No leaderboard data available" when the leaderboard is empty', async () => {
    ...
    render(<AnalyticsPage />);
    await waitFor(() => expect(screen.getByText(/no leaderboard data available/i)).toBeInTheDocument());
  });
