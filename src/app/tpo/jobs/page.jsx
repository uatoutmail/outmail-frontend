  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        logger.error("Failed to fetch jobs data", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <TPOPageShell
        ...
        subtitle="Live job openings tracked by your students, ranked by Outmail Score"
      >
    ...
    >
        ...
        {[
          {
            ...
            label: "Total Openings Tracked",
            value: stats?.totalOpenings?.toLocaleString() || "0",
          ...
          },
          {
            ...
            value: stats?.avgPerStudent || "0",
            sub: "Openings tracked",
            color: "blue",
          },
          {
            icon: Building2,
            label: "Unique Companies",
            value: stats?.uniqueCompanies?.toLocaleString() || "0",
            sub: "With active hiring",
            color: "green",
          },
          {
            icon: Zap,
            label: "Hot Opportunities",
            value: stats?.hotOpportunities?.toLocaleString() || "0",
            sub: "Score ≥ 90 this week",
            color: "orange",
          },
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 p-6 shadow-sm flex flex-col gap-4">
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-1">
              Outmail Score — How It Works
            </h3>
            <p className="text-xs text-gray-400">
              Scores rank each job by how urgently students should reach out
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              {
                label: "Hiring Signal",
                desc: "Recent funding, headcount surge, or new job posted",
                color: "purple",
                weight: "40%",
              },
              {
                label: "Company Momentum",
                desc: "Growth stage, market buzz, press mentions this quarter",
                color: "blue",
                weight: "35%",
              },
              {
                label: "Fit Score",
                desc: "Match between student profile & role requirements",
                color: "green",
                weight: "25%",
              },
              ...
              <div
                key={label}
                className={`border ${tpoColor(color).border100} ${tpoColor(color).bg50} rounded-xl p-4`}
              >
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
                ...
                >
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-lg bg-purple-50 text-purple-700 text-xs font-bold flex items-center justify-center">
                        {j.company[0]}
                      </div>
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${urgencyBadge[j.urgency] || urgencyBadge["Normal"]}`}
                    >
                      {j.urgency}
                    </span>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-5 py-4 text-center text-sm text-gray-400">
                    No jobs found
                  </td>
                </tr>
              )}
        ...
        <div className="px-6 py-3 border-t border-gray-100">
          <p className="text-xs text-gray-400">
            Showing {filtered.length} of {ALL_JOBS.length} tracked openings
          </p>
        </div>
      </div>
    </TPOPageShell>
