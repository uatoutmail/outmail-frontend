...
export const ROWS = [
  ...
  {
    l: "Student effort",
    a: "Blind applications into ATS queues",
    b: "Targeted outreach from their own inbox",
  },
  ...
  return (
    <section className="max-w-4xl mx-auto px-6 py-24">
      <Reveal className="mb-8">
        ...
        <MaskLines
          lines={["Flip the switch."]}
          className="font-syne text-3xl md:text-4xl font-bold tracking-tight mb-3"
        />
        <p className="text-white/45 max-w-lg">
          Same batch, same season. The only variable is whether they have Outmail.
        </p>
      </Reveal>
      <div className="flex justify-center mb-8">
        <div
          className="inline-flex p-1 rounded-pill border border-white/12 bg-white/[0.03]"
          role="tablist"
        >
          {[
            { k: false, l: "Today" },
            { k: true, l: "With Outmail" },
          ].map((o) => (
            <button
              key={o.l}
              role="tab"
              aria-selected={on === o.k}
              onClick={() => setOn(o.k)}
            ...
            >
              ...
              <span className={`relative ${on === o.k ? "text-white" : "text-white/45"}`}>
                {o.l}
              </span>
            </button>
          ))}
      ...
      <div
        className={`rounded-3xl border p-9 transition-colors duration-300 ${on ? "border-primary/35 bg-gradient-to-br from-primary/12 to-transparent" : "border-white/10 bg-white/[0.02]"}`}
      >
        <div className="space-y-7">
          {ROWS.map((r, i) => (
            <div key={r.l} className="grid md:grid-cols-12 gap-4 items-baseline">
              <p className="md:col-span-3 text-[10px] uppercase tracking-[2px] text-white/30">
                {r.l}
              </p>
