...
export default function HeroVisual() {
  ...
  return (
    <div className="relative w-full h-[380px] hidden lg:block" style={{ transformStyle: "preserve-3d" }}>
        ...
        <motion.div {...float(7, 13)}
          ...
          <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
            <motion.div className="h-full rounded-full bg-gradient-to-r from-primary to-accent-light"
              initial={reduce ? false : { width: 0 }} animate={{ width: "94%" }}
      ...
      {!reduce && (
        <motion.svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ transform: "translateZ(20px)" }}>
          <motion.path d="M 120 120 Q 170 190 100 250" fill="none" stroke="url(#g)" strokeWidth="1.5" strokeDasharray="4 5"
            initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: 1, opacity: 0.55 }}
            transition={{ duration: 1.1, delay: 1.1, ease: EASE_OUT }} />
          <defs>
            <linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stopColor="#4c1fff" /><stop offset="100%" stopColor="#a78bfa" />
            </linearGradient>
          </defs>
        </motion.svg>
      )}
