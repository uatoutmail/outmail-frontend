...
function Head({ lines, center = false, sub }) {
  return (
    <Reveal className={center ? "text-center" : ""}>
      ...
      />
      {sub && (
        <p className={`text-white/45 mt-4 ${center ? "max-w-lg mx-auto" : "max-w-lg"}`}>{sub}</p>
      )}
    </Reveal>
  );
}
    ...
    <section className="max-w-4xl mx-auto px-6 py-28">
      <div className="mb-10">
        <Head
          ...
          sub="Pick the thing you are actually worried about."
        />
      </div>
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm transition-colors duration-200 ${
                on
                  ...
                  : "border border-white/12 text-white/50 hover:text-white hover:border-white/30"
              }`}
            >
              <I size={14} />
              {c}
        ...
        >
          {shown.map((f, i) => {
            ...
            return (
              <div
                key={f.q}
                className={`rounded-2xl border transition-colors duration-200 ${isOpen ? "border-primary/45 bg-primary/[0.06]" : "border-white/10 bg-white/[0.02] hover:border-white/25"}`}
              >
                <button
