 ...
 */
export default function PageHeader({ kicker, lines, sub, accentIdx = 1, children, compact = false }) {
  const reduce = useReducedMotion();
  return (
    <header className={`relative px-6 overflow-hidden ${compact ? "pt-16 pb-10" : "pt-20 pb-16 md:pt-24 md:pb-20"}`}>
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        {sub && (
          <Reveal delay={0.25}>
            <p className="text-white/45 text-base md:text-lg leading-relaxed max-w-2xl mt-6">{sub}</p>
          </Reveal>
        )}
        {children && <Reveal delay={0.35}><div className="mt-9">{children}</div></Reveal>}
      </div>
    </header>
  );
}
