...
export default function Features() {
  ...
  return (
    <div className="relative overflow-hidden bg-surface-page flex items-center justify-center px-8 py-20 min-h-screen" style={{

  }}>
      <div className="absolute top-14 right-[8%] w-24 h-24 rounded-full border border-white/10 bg-accent-light/12" />
      <div className="absolute bottom-12 left-[6%] w-20 h-20 rounded-lg border border-white/10 bg-primary/12 rotate-12" />

      <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 gap-16 items-stretch">
        {/* Left Side */}
        <div className="text-white flex flex-col">
          <p className="text-xs font-display font-medium text-accent-light uppercase tracking-[4px] mb-2">Everything You Need to Land the Job</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 flex-1">
            {features.map((feature, idx) => (
              <div key={idx} className="flex flex-col gap-4 h-full p-6 rounded-xl border border-white/12 bg-white/5 backdrop-blur-xl hover:border-purple-500/40 transition-all duration-300 ease-in-out transform hover:scale-[1.02] shadow-[0_0_20px_rgba(108,0,255,0.15)]">
                <div className="bg-primary/30 p-3 w-fit rounded-full border border-purple-500/40">
                  {feature.icon}
                </div>
