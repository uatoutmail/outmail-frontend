...
describe('TPOPageShell', () => {
  it('shows a loading spinner (not the real content) while auth is resolving', () => {
    useAuth.mockReturnValue({ user: null, userRole: null, isAuthenticated: false, loading: true, logout: vi.fn() });
    render(<TPOPageShell><div>Real TPO content</div></TPOPageShell>);
  ...
  it('redirects an unauthenticated visitor to /tpo/login, without rendering content', async () => {
    useAuth.mockReturnValue({ user: null, userRole: null, isAuthenticated: false, loading: false, logout: vi.fn() });
    render(<TPOPageShell><div>Real TPO content</div></TPOPageShell>);
  ...
  it('redirects a signed-in STUDENT to /dashboard, without rendering content', async () => {
    useAuth.mockReturnValue({ user: { role: 'STUDENT' }, userRole: 'STUDENT', isAuthenticated: true, loading: false, logout: vi.fn() });
    render(<TPOPageShell><div>Real TPO content</div></TPOPageShell>);
